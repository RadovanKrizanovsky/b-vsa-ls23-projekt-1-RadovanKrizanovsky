...
public class SanityCheckTest {
  ...
  static void setup() {
    ...
    thesisService =
      (AbstractThesisService<Object, Object, Object>) getServiceClass();
    ...
    thesisClass =
      getEntityClassFromService(
        (Class<AbstractThesisService<?, ?, ?>>) thesisService.getClass(),
        2
      );
    ...
    log.info(
      "Searching for identifier property/field of type Long for entity class " +
      thesisClass.getName()
    );
    thesisIdField = findIdFieldOfEntityClass(thesisClass);
    assertNotNull(thesisIdField);
    log.info(
      "Found identifier for class " +
      thesisClass.getName() +
      " as field '" +
      thesisIdField +
      "'"
    ...
    try {
      log.info(
        "Searching for date type field in " + thesisClass.getName() + " class"
      );
      Object thesis = thesisClass.newInstance();
      Class[] dateClasses = new Class[] {
        Date.class,
        LocalDate.class,
        java.sql.Date.class,
        LocalDateTime.class,
      ...
      Map<Class, List<String>> dateFieldsOfClass = Arrays
        .stream(dateClasses)
        .filter(clazz -> findField(thesis, clazz).size() == 2)
        .collect(
          Collectors.toMap(
            Function.identity(),
            clazz -> (List<String>) findField(thesis, clazz)
          )
        );
      ...
      log.info(
        "Found field for type '" +
        dateFieldsOfClass.keySet().stream().findFirst().get().getName() +
        "' in class " +
        thesisClass.getName()
      );
      List<String> dateFields = dateFieldsOfClass
        .entrySet()
        .stream()
        .findFirst()
        .get()
